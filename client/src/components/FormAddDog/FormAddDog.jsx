...
const validate = (form) => {
    let errors = {}
    if(!form.name) {
        errors.name = "El Nombre es requerido / Name is required"
    }
    if(!form.min_height || !form.max_height) {
        errors.height = "La Altura es requerida / Height is required"
    }
    if(form.min_height <= 0 || form.max_height <= 0){
        errors.height ="Los valores tienen que ser mayores a 0"
    }
    if(!form.min_weight || !form.max_weight) {
        errors.weight = "El Peso es requerido / Weight is required"
    }
    if(form.min_weight <= 0 || form.max_weight <= 0){
        errors.height ="Los valores tienen que ser mayores a 0"
    }
    if(!form.life_span) {
